...
export async function POST(req: Request) {
  ...
  //Role-specific base system instruction
...
Never provide medical advice. Always redirect medical questions to a doctor or nurse.`;
  ...
  //  Build Mem0 conversation payload (last few turns only)
  ...
  //  Write to Mem0 + search relevant memories (with graph enabled globally)
  let memoryContext = "";
  try {
    if (mem0Messages.length > 0 && lastUserText) {
      ...
      // Search Mem0 for relevant memories for this user + agent
  ...
  // Build final system prompt with Mem0 context
...
- If something is unclear or missing, explicitly say what additional information you would need.`;
  ...
  //  Only send a SHORT recent window of turns to the model, not the whole history
